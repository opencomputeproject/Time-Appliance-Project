...
inline std::string get_error_message() {
  ...
  int errsv = errno;
  char buffer[256];
  auto strerr = strerror_r(errsv, buffer, 254);

  str << "BS: errno=" << errsv << ", " << strerr;
  return str.str();
}
...
// Get IPv4/IPv6 info
uint32_t GetAddrFamily();
bool IsIPv4();

class Address {
public:
  Address() : paddr(NULL), p_version(0){};
  Address(unsigned char protocol_version, const std::string &host,
...
public:
   ...
   */
  void sendTo(const void *buffer, int bufferLen, const Address &addr);
