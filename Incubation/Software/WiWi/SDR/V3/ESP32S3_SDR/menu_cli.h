...
extern char print_buffer[2048];


/***** ESP32 S3 CLI functions, sharing CLI between UART0 and USB serial ******/
#define SERIAL_PRINT(x) Serial0.print(x); Serial0.flush(); if (Serial) { Serial.print(x); Serial.flush(); }
#define SERIAL_PRINTLN(x) Serial0.println(x); Serial0.flush(); if (Serial) { Serial.println(x); Serial.flush(); }
