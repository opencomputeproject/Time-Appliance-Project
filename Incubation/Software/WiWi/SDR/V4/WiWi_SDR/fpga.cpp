...
static SoftWire fpga_i2c(FPGA_SDA, FPGA_SCL);
static uint8_t i2c_tx_buf[32];
static uint8_t i2c_rx_buf[32];
static uint8_t fpga_i2c_address = 0x10;


bool fpga_i2c_read(uint16_t address, uint8_t * val)
{
  uint8_t num_read = 0;
  fpga_i2c.beginTransmission(fpga_i2c_address);
  fpga_i2c.write( (uint8_t) ( (address >> 8) & 0xff ) );
  fpga_i2c.write( (uint8_t) (address & 0xff) );
  fpga_i2c.endTransmission(false);
  delayMicroseconds(5);

  num_read = fpga_i2c.requestFrom(fpga_i2c_address,1);
  if ( num_read == 0 ) {
    Serial.println("Failed to read 1 byte from FPGA");
    return 0;
  }
  if ( val != 0 ) {
...
bool fpga_i2c_write(uint16_t address, uint8_t val)
{
  fpga_i2c.beginTransmission(fpga_i2c_address);
  fpga_i2c.write( (uint8_t) ( (address >> 8) & 0xff ) );
  fpga_i2c.write( (uint8_t) (address & 0xff) );
...
void onFpgaI2cWrite(EmbeddedCli *cli, char *args, void *context)
{
  ...
  uint16_t address = 0;
  uint8_t value = 0;
  if ( !try_parse_hex_uint16t( embeddedCliGetToken(args, 1), &address ) ) {
