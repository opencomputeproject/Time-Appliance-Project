...
void onSX1276SendPackets(EmbeddedCli *cli, char *args, void *context)
{
  int packet_count = 100;
  if ( embeddedCliGetTokenCount(args) == 1 ) {
    packet_count = atoi( embeddedCliGetToken(args, 1) );
    sprintf(print_buffer, "Sending %d LoRA packets: %d\r\n", packet_count); Serial.print(print_buffer);
  } 

  const uint8_t pkt_data[] = "hellolora1234567";

  sprintf(print_buffer, "Packets sent: %d", 0); Serial.print(print_buffer); Serial.flush();

  // hard code antenna select, make a better function later
  // 1 => RF1, 0 => RF2
  wwvb_digital_write(LORA_LF_TXRX_SEL, 0); // set to RX, doesn't matter
  wwvb_digital_write(LORA_HF_TXRX_SEL, 1); // set to TX
  wwvb_digital_write(LORA_LF_HF_SEL, 0); // set to HF

  for ( int i = 0; i < packet_count; i++ ) {

    if ( i % 5 == 0) {
      sprintf(print_buffer, "\rPackets sent: %d", i); Serial.print(print_buffer); Serial.flush();
    }
    ...
    SX1276_Lora.write( pkt_data, 16 );
    SX1276_Lora.endPacket(0); // finish packet and send it, blocking
  }
  Serial.println("");
}
...
static Node sx1276_send_packets_node = { .name = "send_packets", 
  .type = MY_FILE, 
  .cliBinding = {
    "send_packets",
    "Send a number of LoRA packets, pass number to send, default 100",
    true,
    nullptr,
    onSX1276SendPackets
  }
