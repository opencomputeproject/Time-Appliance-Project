        ...
        try:
            with open(os.path.join(i2c_bus_path, 'name'), 'r') as file:
                ...
                if adapter_name in name:
                    ...
                    bus_number = int(i2c_bus_path.split('/')[-1].replace('i2c-', ''))
...
class miniptm_i2c:
    ...
    def write_dpll_reg(self, base_addr, offset, value):
        ...
        if self.cur_base_addr != base_addr:
            self.bus.write_i2c_block_data(self.DPLL_ADDRESS, 0xfc, [baseaddr_lower, baseaddr_upper, 0x10, 0x20])
    ...
    def write_dpll_multiple(self, addr, data_bytes):
        ...
        self.bus.write_i2c_block_data(self.DPLL_ADDRESS, 0xfc, [baseaddr_lower, baseaddr_upper, 0x10, 0x20])
        self.bus.write_i2c_block_data( self.DPLL_ADDRESS, baseaddr_lower, data_bytes )
    ...
    def read_sfp_module(self, sfp_num=1):
        ...
        data_a0 = self.read_i2c_data(SFP_ADDRESS_A0, 0, 96)
        vendor_name, serial_number, vendor_part_number, module_type = interpret_data(data_a0)
        print(f"Vendor Name: {vendor_name}, Serial Number: {serial_number}")
        print(f"Vendor Part Number: {vendor_part_number}, Module Type: {module_type}")

        # Read temperature, Tx and Rx power from address A2
        temp_data = self.read_i2c_data(SFP_ADDRESS_A2, 96, 2)  # Assuming temperature is at register 96
        tx_power_data = self.read_i2c_data(SFP_ADDRESS_A2, 102, 2)  # Tx power at register 102
        rx_power_data = self.read_i2c_data(SFP_ADDRESS_A2, 104, 2)  # Rx power at register 104
        ...
        if tx_power_data:
            tx_power_raw = struct.unpack('>H', bytes(tx_power_data))[0]
            tx_power = tx_power_raw * 0.1  # Conversion based on specification
            print(f"Transmit Power: {tx_power} dBm")

        if rx_power_data:
            rx_power_raw = struct.unpack('>H', bytes(rx_power_data))[0]
