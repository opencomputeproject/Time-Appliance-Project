...
ip netns exec ${node[$node_number,0]} phc2sys -O 0 -m -s CLOCK_REALTIME -c ${node[$node_number,$(($(($(($destination_port-1))*2))+1))]} | tee $(basename "$PWD")-PHCSYSto$destination_port.log
